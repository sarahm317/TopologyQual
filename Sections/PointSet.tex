...
\begin{prob}{(F20.2)}{F20.2}
Let $X$ be a topological space. Show that the intersection of any two dense open sets in $X$ is also dense. Give an example that shows this fails without the assumption that the sets are open.
\end{prob}

\begin{solution}
    Both $\Q$ and $\R \sm \Q$ are dense in $\R$, however their intersection is nonempty and therefore not dense in $\R$.
\end{solution}

\begin{proof}
    Let $U, V \sq X$ be two open dense subsets of $X$. Then $U \cap V$ is nonempty since the intersection of $U$ with any open set in $X$ must be nonempty. \\
    
    Let $W$ be an arbitrary open subset of $X$. Then, $U \cap W \neq \varnothing$ since $U$ is dense in $X$. Noting that $U \cap W$ is an open set, $V \cap (U \cap W) \neq \varnothing$ since $V$ is dense in $X$. Then, $(U \cap V) \cap W \neq \varnothing$, proving that $U \cap V$ is dense in $X$.
\end{proof}
...
\begin{proof}
    Let $X = [0,1]$ be an interval in $\R$. Seeking a contradiction, assume that $X = A \cup B$ is a separation. Let $a \in A$ and $b \in B$ be arbitrary. Without loss of generality, assume that $0 \leq a < b \leq 1$. Since $X$ is bounded, let $S = \{x \in A: x < b \}$ and define $\alpha = \sup S$.\\
    
    By assumption $a \in S$ and therefore $\alpha \geq a$. Furthermore, each $x \in S$ satisfies $x < b$ meaning that $\alpha \leq b$. That is, $\alpha \in [a,b]$.\\

    Suppose first that $\alpha \in A$. Because $A$ is open, there exists $r > 0$ such that $(\alpha - r, \alpha + r) \sq A$. This means that $\alpha + \frac{r}{2} \in A$. Since $b \not\in A$ and $b \geq \alpha$, it must be the case that $b > \alpha + r > \alpha + \frac{r}{2}$. Therefore, $\alpha + \frac{r}{2} \in S$, contradicting the fact that $\alpha$ is an upper bound of $S$.\\

    On the other hand, $\alpha$ can be written as the limit of a sequence of points in $A$ and as $A$ is closed, $\alpha \in A$.
\end{proof}

\begin{proof}
    Let $(X,d)$ be a connected metric space with at least two points. Let $x,y \in X$ be distinct points and let $R = d(x,y)$.

    \begin{claim}
        For each $r \in (0,R)$ there exists $z \in X$ such that $d(x,z) = r$.

        \begin{proof}
            We proceed by contradiction. Let $r \in (0,R)$ and suppose that for each $z \in X$, $d(x,z) \neq r$. Define $U = B_r(x)$ to be the open ball of radius $r$ centered at $x$. From the assumptions, it follows that $B_r(x) = \ol{B_r(x)}$ since the boundary of the ball is empty. Therefore, $V = X \sm \ol{B_r(x)}$ is open and satisfies $X = U \cup V$. Since $d(x,y) = R > r$, $y \in V$. Then $U\cup V$ form a separation of $X$, a contradiction.
        \end{proof}
    \end{claim}

    Since $(0,R)$ is uncountable, the claim implies that $X$ is uncountable.
\end{proof}

\begin{prob}{(F13.3)}{F13.3}
