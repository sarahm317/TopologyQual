...
\begin{proof}
    ...
    Now assume that $X$ and $Y$ are both compact. Let $\scr{U} = \{U_\alpha\}_{\alpha \in \scr{A}}$ be an open cover of $X \times Y$. Fix $y \in Y$ and note that $X \times \{y\} \cong X$. Therefore, $X \times \{y\}$ is compact. Since $\scr{U}$ is also an open cover of $X \times \{y\}$, there exists a finite subcover, say $U_1,\ldots, U_n$. 
\end{proof}
