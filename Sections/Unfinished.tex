...
\begin{proof}
    Let $\{X_\alpha\}$ be a collection of connected sets such that each $X_\alpha \cap X_{\alpha'} \neq \varnothing$. Let $X = \bigcup_\alpha X_\alpha$. Seeking a contradiction, suppose that $C \cup D = X$ is a separation. Because each $X_\alpha$ is connected, either $X_\alpha \sq C$ or $X_\alpha \cap C = \varnothing$. If this were not the case, then intersecting $C$ with $X_\alpha$ would result in a separation of $X_\alpha$.\\

    Suppose that $X_\alpha \sq C$ and $X_\beta \cap C = \varnothing$ for some $\alpha \neq \beta$ in the indexing set. By assumption, there exists $p \in X_\alpha \cap X_\beta$ which is impossible. Therefore every $X_\alpha \sq C$ meaning that $D$ is empty. Thus $X$ is connected.
\end{proof}
